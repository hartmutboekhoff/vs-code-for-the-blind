...
class ViewGroup extends ValueGroupWrapper {
  constructor(obj, schema, key, path, status) {
    ...
    this.children.append(new DropdownInputLine(path+'.type', schema.properties?.type?.title ?? 'Typ', obj.type, schema.properties?.type?.enum, schema.properties?.type?.description),
                         new TextInputLine(path+'.id', schema.properties?.id?.title ?? 'ID', obj.id, schema.properties?.id?.description));
  }
